...
parser.add_argument('--basepath', type=str, default='/data1/bks/wumengke/model_weight/Qwen2-VL-2B-Instruct')
...
from model.modeling_qwen2_2b import Qwen2VLModel
#from ..model.configs import EConfig
from typing import Any, Dict, List
from model.config_yarn_new import Qwen2VLConfig
...
def getkacc(model, data, head, max_length=5):
    def generate(hidden_states, input_ids, head,kv_cache, max_length=4):
        # if use_cache:
        #     past_key_values = None
        #     for i in range(max_length):
        #         if past_key_values != None:
        #             out_hidden, past_key_values = model(last_hidden, input_ids=token, kv_cache=,
        #                                                 use_cache=True)
        #         else:
        ...
        for i in range(max_length):
            if kv_cache is not None:
                out_hidden = model(last_hidden,input_ids=token,kv_cache=kv_cache)
            else:
                out_hidden = model(hidden_states,input_ids=input_ids,kv_cache=kv_cache)
            last_hidden = out_hidden[:,-1:]
            last_headout = head(last_hidden)
            token = torch.argmax(last_headout,dim=-1)
            input_ids = torch.cat((input_ids,token),dim=1)
...
model = Qwen2VLModel(config, load_emb=True, path=args.basepath)
