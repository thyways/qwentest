...
class InferenceEngine:
    ...
    @torch.inference_mode()
    def new_token_model_run(self, input_ids: torch.LongTensor,pixel_values,image_grid_thw):
        if input_ids.shape[-1] > 64: # prefill
            # iter_prefill = math.ceil(input_ids.shape[1] / 128)
            # for i in range(iter_prefill):
            if pixel_values is not None and image_grid_thw is not None:
                #logits = self.model(input_ids=input_ids,pixel_values=pixel_values,image_grid_thw=image_grid_thw).logits
                logits = self.model(input_ids=input_ids,pixel_values=pixel_values,image_grid_thw=image_grid_thw,kv_cache=self.kv_cache,graph_cache=None,).logits
            else:
    ...
    @torch.inference_mode()
    def draft_run(self, input_ids: torch.LongTensor,pixel_values,image_grid_thw, gamma_offset: int=0, probs=False, temperature=0.6, top_p=0.9,):
        if input_ids.shape[-1] > 64: # prefill
            logits = self.draft(input_ids=input_ids,pixel_values=pixel_values,image_grid_thw=image_grid_thw,kv_cache=self.draft_cache,graph_cache=None).logits
        else: # decoding
        ...
        if probs: # without top_p
            return norm_logits(logits[0], temperature=0.6, top_k=-1, top_p=top_p)[-1]
        return logits
    ...
    def model_verify(self, input_ids: torch.LongTensor, probs=False, temperature=0.6, top_p=0.9):
        ...
        logits = self.model(input_ids=input_ids, kv_cache=self.kv_cache,graph_cache=self.graph_cache, spec=True).logits
...
def draft_run_capture_graph(engine :InferenceEngine, gamma_offset :int =0, mempool=None, n_warmups :int=3, probs=False, temperature=0.6, top_p=0.9):
    ...
    with torch.cuda.stream(s):
        for _ in range(n_warmups):
            static_logits = engine.draft_run(input_ids=static_input_ids, pixel_values=None,image_grid_thw=None,gamma_offset=gamma_offset, probs=probs, temperature=temperature, top_p=top_p)
        s.synchronize()
...
def model_verify_capture_graph(engine :InferenceEngine, mempool=None, n_warmups :int=3, gamma:int=6, probs=False, temperature=0.6, top_p=0.9):
    ...
    static_position_ids = torch.arange(gamma+1, device=device).unsqueeze(0).unsqueeze(0)
...
class GraphInferenceEngine:
    ...
    def graph_draft_prefill(self, input_ids: torch.LongTensor,pixel_values=None,image_grid_thw=None):
        # draft run
        logits = self.engine.draft_run(input_ids=input_ids,pixel_values=pixel_values,image_grid_thw=image_grid_thw)
        return logits
    ...
    def new_token_inference(self, input_ids,pixel_values=None,image_grid_thw=None):
        return self.engine.new_token_model_run(input_ids,pixel_values,image_grid_thw)
