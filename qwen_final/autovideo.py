...
draft_model = Qwen2VLForConditionalGeneration.from_pretrained('/data1/bks/wumengke/model_weight/Qwen2-VL-2B-Instruct',torch_dtype = torch.float16,device_map="cuda:2",attn_implementation="flash_attention_2")
...
def autoregressive_sampling(inputs,model,cache,max_len):
    ...
    return generated_text,(time2-time1)
output,lentancy = autoregressive_sampling(inputs=inputs,model=draft_model,cache=cache,max_len=20)
print(output)
print(lentancy)
