...
def original_sampling( model, input_tokens, max_len):
    generate_ids = model.generate(**input_tokens, max_new_tokens=max_len)
    return generate_ids
    

@torch.inference_mode()
def Autoregressive(processor, graph_engine, inputs, max_len=256, top_k=-1, top_p=0.9, temperature=0.6, verbose=False):
    # reset all cache
    graph_engine.engine.kv_cache.reset()
    logits = graph_engine.full_inference(input_ids=inputs.input_ids,pixel_values_videos=inputs.pixel_values_videos,video_grid_thw=inputs.video_grid_thw,)
    ...
    time1 = time.time()
    while n < max_len:
        logits = graph_engine.full_inference(input_ids=next_token,pixel_values_videos=None,video_grid_thw=None)
        next_token = sample(norm_logits(logits[:,-1,:], temperature=temperature ,top_k=top_k, top_p=top_p))
        n += 1
        if verbose:
            spec_stream(next_token[0], processor, 'cyan')
...
@torch.inference_mode()
def TriForce(processor, graph_engine, inputs, gamma=4, max_len=256, top_k=-1, top_p=0.9, temperature=0.6, verbose=False, file_path=None, dataset=None, spec_args=None):
    ...
    logits = graph_engine.full_inference(input_ids=inputs.input_ids,pixel_values_videos=inputs.pixel_values_videos,video_grid_thw=inputs.video_grid_thw,)
    _ = graph_engine.retrieval_inference(input_ids=inputs.input_ids,pixel_values_videos=inputs.pixel_values_videos,video_grid_thw=inputs.video_grid_thw,)
    _ = graph_engine.graph_draft_prefill(input_ids=inputs.input_ids)
    ...
    while n < max_len:
        ...
        logits = graph_engine.full_inference(input_ids=verify_tokens,pixel_values_videos=None,video_grid_thw=None)
        ...
        pass_tokens = torch.full((1, gamma2 + 2), 0, device=graph_engine.engine.model.device)
        ...
        graph_engine.engine.kv_cache.reset_kv_cache()
        graph_engine.engine.graph_cache.seq_len -= (gamma2+1)
        graph_engine.engine.graph_cache.reset_graph_cache()
        graph_engine.engine.graph_cache.update(graph_engine.engine.kv_cache,len(generated_ids) - count)
    ...
    if verbose:
        print(f"\nUse {time2 - time1} sec to generate {n} tokens (now {graph_engine.engine.kv_cache.seq_len} tokens), Tokens/s: {n / (time2 - time1)}", flush=True)
    ...
    return acceptance_rate, n / (time2 - time1),acc_rate_middle_list
...
def Middle_Spec(next_token, graph_engine, gamma, verbose, tokenizer):
    ...
    verify_tokens = torch.full((1, gamma + 1), 0, device=graph_engine.engine.model.device)
    ...
    while n < gamma:
        ...
        verify_prob = graph_engine.engine.model_verify(input_ids=verify_tokens,probs=True, temperature=0.01, top_p=0.9)
        ...
        if r < torch.min(torch.tensor([1], device=r.device), (verify_prob[n, token_idx] / speculation_prob[token_idx])):
            ...
            verify_tokens[:, n:n+1] = pred_token_idx
        if n < gamma:
            graph_engine.engine.graph_cache.seq_len -= (gamma+1)
            graph_engine.engine.graph_cache.reset_graph_cache()
